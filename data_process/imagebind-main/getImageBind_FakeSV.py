...
# 获取所有需要的文件
with open("/MSAF_Net/data/FakeSV/data-split/all.txt", "r") as file:
    lines = file.readlines()
all_vid = [line.strip() for line in lines]
batch = 32

audio_folder = '/MSAF_Net/data/FakeSV/vids_mp3'  # 替换为你的音频文件夹路径
video_folder = '/MSAF_Net/data/FakeSV/vids'  # 替换为你的图片文件夹路径
break_video_folder = '/MSAF_Net/data/FakeSV/fix_vids'  # 替换为你的图片文件夹路径
...
with torch.no_grad():
    ...
    filename = '/MSAF_Net/data/FakeSV/title_chinese_withLbael_Data_cleaning.json'
    ...
    for i in all_vid:
        if i in break_videos:
            image_paths.append(break_video_folder + "/fixed_" + i + ".mp4")
        else:
    ...
    torch.save(all_clip_feature, "/MSAF_Net/fea/fakesv/preprocess_clip/clip_imagebind_data_clearing_fea.pkl")
